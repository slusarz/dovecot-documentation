...
.. Note:: You can't currently use ``doveadm metacache clean`` to delete changed
          indexes. The only alternative is to just forcibly ``rm -rf`` the
	  directory. However, if the user happens to be accessed during the
	  ``rm -rf`` this can cause index corruption, which can have rather
	  bad consequences (like redownloading all mails). This is why it
	  should verify whether this is the user's currently assigned backend,
	  and only ``rm -rf`` users whose backend is elsewhere.
...
.. dovecotadded:: 2.4.0,3.0.0 To allow easier migration of users and to support the new needs brought up with
   the dovecot_cluster_architecture the ``doveadm metacache pull``
