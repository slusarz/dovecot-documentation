...
export async function dovecotMdInit() {
	...
	md_conf = {
		...
		man: await loadManFiles(),
		plugins: await loadPluginFiles()
	}

	return md_conf
}

async function loadManFiles() {
	const paths = dovecotSetting('man_paths')
	const files = await Promise.all(paths.map(x => fg(x)))
	return files.flat().map((y) => {
...
async function loadPluginFiles() {
	const paths = dovecotSetting('plugin_paths')
	const files = await Promise.all(paths.map(x => fg(x)))
	return files.flat().map((y) => path.basename(y, '.md'))
}
