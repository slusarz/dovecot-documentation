...
export async function dovecotMdInit() {
	...
	/* Load bootstrap data asynchronously. */
	const [doveadm, events, settings, updates, links] = await Promise.all([
		loadData('doveadm'),
		loadData('events'),
		loadData('settings'),
		loadData('updates'),
		(async () => {
			...
			await frontmatterIter(Object.keys(rewrites), function (f, data) {
	...
	md_conf = {
		doveadm: doveadm.doveadm,
		doveadm_man: new Set(Object.values(doveadm.doveadm).filter(x => x.man).map(x => x.man)),
		...
		case 'setting_text':
			...
			case 'doveadm':
				if (!opts.doveadm[env.inner]) {
					if (!opts.doveadm_man.has('doveadm-' + env.inner)) {
