...
function processInherit(i, ob) {
	let fields = {}
	const event = ob.events[i] || ob.inherits[i]

	if (event) {
		fields = { ...fields, ...normalizeEventsFields(event.fields || {}) }
		if (event.inherit) {
			fields = { ...fields, ...processInheritList(event.inherit, ob) }
		}
	}
