...
export async function getExcludes(srcDirs = [ 'docs' ]) {
	const excludes = []

	await frontmatterIter(
		(await Promise.all(srcDirs.map((x) => fg(x + '/**/*.md')))).flat(),
		function (f, data) {
...
export async function frontmatterIter(files, callback) {
