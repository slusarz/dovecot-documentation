...
- [Mail Storage](mail_storage) is used to access mails
  in a specific location with a specific [[link,design_mailbox,mailbox]] format. Multiple namespaces
  can point to the same storage. A single namespace may in future (but not
  currently) point to multiple storages (e.g. a mixed mbox and [[link,maildir,Maildir]]
  directory).
