...
Currently this is only used by [[link,mbox,mbox]] format to specify the address in From-line.
...
may not be given. For example with [[link,maildir,maildir]] the GUID is same as the base
filename, while [[link,dbox,dbox]] has an explicit GUID metadata field.
...
Set [[link,pop3,POP3]] UIDL value. Not all backends support this.
