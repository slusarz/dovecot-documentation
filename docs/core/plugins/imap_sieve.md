...
The `imapsieve` extension ([[rfc,6785]]) defines environment items for the `environment` extension ([[rfc,5183]]). Beyond those, Dovecot provides additional environment items via the `vnd.dovecot.imapsieve` extension (enabled with `require "vnd.dovecot.imapsieve";` or implicitly when `require "imapsieve";` is used):
